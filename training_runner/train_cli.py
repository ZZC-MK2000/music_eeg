...
def _add_train_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--model-variant", type=str, default="all", choices=["all", "resmlp", "msresnet", "chanattn"])
...
def _run_train(args: argparse.Namespace, *, epochs: int | None = None, patience: int | None = None) -> None:
    ...
    reports_json_dir = output_dir / "reports" / "json"
    models_sklearn_dir = output_dir / "models" / "sklearn"
    models_encoder_dir = output_dir / "models" / "encoders"
    ...
    model_path = models_sklearn_dir / f"model_{run_name}.joblib"
    encoder_path = models_encoder_dir / f"encoder_{run_name}.pkl"
    report_path = reports_json_dir / f"report_{run_name}.json"
